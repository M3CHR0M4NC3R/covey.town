import { Button, Container, List, ListItem, useToast, Input } from '@chakra-ui/react';
...
export default function MusicArea({
  ...
  return (
    <Container id='containerBrowseSongs' style={{width: '100%', margin: 'unset', padding: 'unset'}}>
        <div id="lookupContainer" style={{display: 'flex', flexDirection: 'row', justifyContent: 'space-between', padding: '0.5em 0 0.5em 0', borderTop: 'solid', borderBottom: 'solid'}}>
            <div id="lookupSongContainer">
                <label htmlFor="song">Lookup Song: </label>
                <input type="text" name="song" id="song" onKeyDown={handleKeyDown} onKeyUp={handleKeyUp} style={{backgroundColor: '#bababa', minHeight: '100%'}}/>
            </div>
            <div className="topBtnContainer" style={{display: 'flex', flexDirection: 'row'}}>
              <button id="searchBtn" type='submit' typeof='name' onClick={() => {console.log('TODO: Search the database for the title!')}} style={{padding: 'revert', backgroundColor: '#bcebb6'}}>Search!</button>
              <button id="toCreation" onClick={() => {setRoute('creation')}} style={{padding: 'revert', backgroundColor: '#d4a301'}}>Create Song!</button>
            </div>
        </div>
        <div id="songsContainer" style={{display: 'flex', flexDirection: 'column'}}>
            {songs?.map((song, index) => (
                <div key={index} id={`songContainer-${index}`} style={{borderBottom: '1px solid grey', padding: '0.5em 0 0.5em 0', display: 'grid', gridTemplateColumns: '0.4fr 1.3fr 1.3fr', gridTemplateRows: '1fr 1fr', gap: '0.5em 0.5em', gridTemplateAreas: `'btnView title creator' 'btnView desc likes'`}}>
                    ...
                    <p id='likes' className='likes' style={{height: '50%', gridArea: 'likes'}}><b>Likes:</b> {song.likes}</p>
                </div>
            ))}
        </div>
    </Container>
